...
public class Test extends JApplet implements Runnable, MouseListener, MouseMotionListener {
   ...
   private class Point { // the object class, also sometimes used to pass x y corrdinates
      ...
      public Point(double x, double y) {
         ...
         this.c = new Color(rnd.nextInt(256), rnd.nextInt(256), rnd.nextInt(256));
   ...
   public void init()
   {
      ...
      mouse[MOUSE] = new Point(NOWHERE, NOWHERE);
      mouse[MOUSE].setImage(pics[6].getScaledInstance(50, 50, 0));
      for (int i = MOUSE + 1; i < numPeople; i++) {
         mouse[i] = new Point(rnd.nextInt(WIDTH - 50), rnd.nextInt(HEIGHT - 50));
         mouse[i].setImage(pics[0].getScaledInstance(50, 50, 0));
         mouse[i].speedX = (rnd.nextInt(2) * 2 - 1) * this.simulationSpeed;
         mouse[i].speedY = (rnd.nextInt(2) * 2 - 1) * this.simulationSpeed;
      }
   ...
   */
   public void run(){
      while(true){
         ...
         for (int i = MOUSE + 1; i < numPeople; i++) {
            this.mouse[i].update();
            if (this.mouse[i].y >= HEIGHT - this.mouse[i].img.getHeight(this) || this.mouse[i].y <= 0) {
               this.mouse[i].changeDirectionY();
            }
            if (this.mouse[i].x >= WIDTH - this.mouse[i].img.getWidth(this) || this.mouse[i].x <= 0) {
               this.mouse[i].changeDirectionX();
            }
         }
         ...
         for (int i = 0; i < numObject; i++) {
            ...
            /*if (!list[i].multiCollide && collide.size() > 1) {
               ...
               list[i].multiCollide = true;
            }
            if (checkCollide(list[i], this.mouse) != 0) {
               collide.add(checkCollide(list[i], this.mouse));
            }*/
            ...
            if (list[i].y <= 0) {
               list[i].y = 0;
            }/*else if (!collide.isEmpty() && !collide.contains(HORIZONTAL_COLLIDE) || collide.size() > 1) { //!=1
               list[i].changeDirectionY();
            }*/
            ...
            if (list[i].x <= 0) {
               list[i].x = 0;
            }/*else if (!collide.isEmpty() && !collide.contains(VERTICAL_COLLIDE) || collide.size() > 1) {
               list[i].changeDirectionX();
            }*/
            boolean first = true;
            ...
            for (int p = 0; p < this.numPeople; p++) {
               ...
               double distance = this.getDistance(this.list[i], this.mouse[p]);
            
               if (distance < this.radius  - this.radius / 10) {
                  check = false;
                  Point v = this.getVelocity(this.list[i], this.mouse[p], AWAY);
                  ...
                  if (x != 0 && y != 0 && p == numPeople - 1 && check) {
         ...
         for (int i = MOUSE + 1; i < numPeople; i++) {
            g.drawImage(this.mouse[i].img, (int) this.mouse[i].x, (int) this.mouse[i].y, this);
         }
         ...
         for (int i = 0; i < numFrog; i++) {
            ...
            for (int p = 0; p < this.numPeople; p++) {
               double distance = this.getDistance(this.mouse[p], this.frog[i]);
   ...
   */
   ...
   public void mouseMoved(MouseEvent e ) {  // called during motion when no buttons are down
      this.mouse[MOUSE].x = e.getX();
      this.mouse[MOUSE].y = e.getY();
      e.consume();
   }
   ...
   public void mouseExited(MouseEvent e) {
      ...
      this.mouse[MOUSE].x = NOWHERE;
      this.mouse[MOUSE].y = NOWHERE;
   }
